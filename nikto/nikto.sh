...
if [[ $choice -eq 0 ]]; then
echo "Standard Nikto Scanning"
 nikto -h $ip -output $output
...
nikto -h $ip -Tuning 0  -output $output
elif [ "$option" -eq 1 ]
then
echo "Interesting File Tuning"
nikto -h $ip -Tuning 1 -output $output
elif [ "$option" -eq 2 ]
then 
echo "Misconfiguration" 
nikto -h $ip -Tuning 2  -output $output


elif [ "$option" -eq 3 ]
then
echo "Injection(XSS/Script)"
nikto -h $ip -Tuning 3 -output $output

elif [ "$option" -eq 4 ]
then
echo "Remote File Retrieval(Inside Web root)"
nikto -h $ip -Tuning 4 -output $output

elif [ "$option" -eq 5 ]
then
echo "Denial Of Service"
nikto -h $ip -Tuning 5 -output $output

elif [ "$option" -eq 6 ]
then
echo "Remote File Retrieval(Server Wide)"
nikto -h $ip -Tuning 6 -output $output

elif [ "$option" -eq 7 ]
then
echo "Command Execution"
nikto -h $ip -Tuning 7 -output $output

elif [ "$option" -eq 8 ]
then
echo "SQL Injection"
nikto -h $ip -Tuning 8 -output $output
